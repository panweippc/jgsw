    ...
    <ElTable :data="tableData" :loading="loading" border style="width: 100%; margin-top: 20px;">
      ...
      <ElTableColumn label="操作" width="200">
        <template #default="scope">
          ...
          <ElButton size="small" type="warning" @click="handleResetPassword(scope.row.id!)">重置密码</ElButton>
      ...
      <ElForm :model="form" label-width="100px">
        ...
        <ElFormItem label="所属部门">
          <ElSelect v-model="form.deptId" placeholder="请选择部门">
            <ElOption v-for="dept in flattenDepts(depts)" :key="dept.id" :label="dept.deptName" :value="dept.id" />
          </ElSelect>
        </ElFormItem>
        <ElFormItem label="岗位">
          <ElSelect v-model="form.postId" placeholder="请选择岗位">
            <ElOption v-for="post in posts" :key="post.id" :label="post.postName" :value="post.id" />
          </ElSelect>
        </ElFormItem>
        ...
        <ElFormItem label="角色">
          <ElSelect v-model="form.roleIds" multiple placeholder="请选择角色">
            <ElOption v-for="role in roles" :key="role.id" :label="role.roleName" :value="role.id" />
          </ElSelect>
        </ElFormItem>
      </ElForm>
      <template #footer>
