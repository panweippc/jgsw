      ...
      <ElTableColumn label="操作" width="200" fixed="right">
        <template #default="scope">
          <div class="action-buttons">
            <ElButton size="small" @click="fetchItems((scope.row as SysDictType).id!)">查看项</ElButton>
            <ElButton size="small" @click="typeForm = { ...(scope.row as SysDictType) }; typeDialogVisible = true">编辑</ElButton>
            <ElButton size="small" type="danger" @click="handleDeleteType((scope.row as SysDictType).id!)">删除</ElButton>
          </div>
        </template>
      </ElTableColumn>
    </ElTable>
