...
import { ElTable, ElTableColumn, ElButton, ElDialog, ElForm, ElFormItem, ElInput, ElMessage, ElPagination, ElSelect, ElOption } from 'element-plus'
...
function handleAdd() { 
  form.value = { materialCode: '', materialName: '', status: 1, categoryId: undefined, spec: '', unit: '', purchasePrice: 0 }
...
<template>
  <div class="page-container">
    ...
    <ElTable :data="tableData" border style="width: 100%; margin-top: 20px;">
      ...
      <ElTableColumn prop="purchasePrice" label="采购单价" />
      <ElTableColumn prop="retailPrice" label="零售单价" />
      ...
      <ElTableColumn label="操作" width="150" fixed="right">
        <template #default="scope">
          <div class="action-buttons">
            <ElButton size="small" @click="handleEdit(scope.row as Material)">编辑</ElButton>
            <ElButton size="small" type="danger" @click="handleDelete((scope.row as Material).id!)">删除</ElButton>
          </div>
        </template>
      </ElTableColumn>
    </ElTable>
      ...
      <ElForm :model="form" label-width="100px">
        ...
        <ElFormItem label="分类">
          <ElSelect v-model="form.categoryId" placeholder="请选择分类">
            <ElOption v-for="cat in categories" :key="cat.id" :label="cat.categoryName" :value="cat.id || 0" />
          </ElSelect>
        </ElFormItem>
        <ElFormItem label="规格"><ElInput v-model="form.spec" /></ElFormItem>
        <ElFormItem label="单位"><ElInput v-model="form.unit" /></ElFormItem>
        <ElFormItem label="采购单价"><ElInput v-model.number="form.purchasePrice" type="number" /></ElFormItem>
        <ElFormItem label="零售单价"><ElInput v-model.number="form.retailPrice" type="number" /></ElFormItem>
      </ElForm>
      <template #footer>
